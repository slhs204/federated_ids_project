"""
Gradio Deployment Interface for FL-IDS (Enhanced - Full Dataset Analysis)
...
def analyze_full_dataset(csv_file, max_samples=None):
    ...
    try:
        ...
        if max_samples and len(df) > max_samples:
            ...
            print(f"ℹ️  Sampled {max_samples:,} from {len(df):,} rows")
...
def predict_single_row(csv_file):
    """Quick single-row prediction (original functionality)"""
    try:
        df = pd.read_csv(csv_file.name)
...
Upload network flow data to analyze entire datasets or test single samples.
        ...
        with gr.Tabs():
            ...
            with gr.Tab("📊 Full Dataset Analysis"):
                gr.Markdown("""
### Batch Analysis
Analyze entire CSV files with thousands of samples. Generates:
...
- Downloadable results
                ...
                with gr.Row():
                    ...
                    max_samples_input = gr.Number(
                        label="Max Samples (leave empty for all)",
                        value=None,
                        precision=0
                    )
            ...
            with gr.Tab("🔍 Single Row Analysis"):
                gr.Markdown("Quick analysis of first row (for testing)")
            ...
            with gr.Tab("⌨️ Manual Input"):
                gr.Markdown("Enter features manually for quick testing")
...
**Hardware:** MSI Vector 16 HX (RTX 5070 Ti)
        ...
        gr.Markdown("---\n**Note:** Automatically handles any feature dimension and dataset size.")
