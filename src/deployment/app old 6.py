"""
Gradio Deployment Interface for FL-IDS (Final Corrected)
...
def predict_from_csv(csv_file):
    ...
    try:
        ...
        if len(numeric_df.columns) == 0:
            return "❌ No numeric columns found in CSV", None, None
        ...
        # Get first row as numpy array
        features = numeric_df.iloc[0].values.astype(float)
        
        # Handle NaN/Inf
        features = np.nan_to_num(features, nan=0.0, posinf=1e6, neginf=-1e6)
        
        print(f"📊 CSV: {num_features} numeric features detected")
...
**Dataset:** {num_features} numeric features detected  
...
def create_interface():
    ...
    with gr.Blocks(title="FL-IDS: Federated Intrusion Detection", theme=gr.themes.Soft()) as demo:
        ...
        gr.Markdown(
            """
            ---
            **Note:** System automatically handles any number of numeric features (padding/truncation as needed).
            """
        )
